...
resource "platform-orchestrator_provider" "google" {
  id                 = "${var.prefix}-google"
  description        = "Google provider for the temporary use case ${var.prefix}"
...
resource "platform-orchestrator_module" "postgres_instance" {
  ...
  module_source = "git::https://github.com/humanitec-tutorials/get-started//modules/postgres/gcp" # TODO - move into use case repo or make inline
  provider_mapping = {
    google = "${platform-orchestrator_provider.google.provider_type}.${platform-orchestrator_provider.google.id}"
  }
}
